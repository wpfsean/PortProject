...
public class ReceiveEmergencyAlarm  implements Runnable {


    EmergencyAlarmText callback;

    public ReceiveEmergencyAlarm ( EmergencyAlarmText callback){
    ...
    public void run() {
        ...
        try {
            ServerSocket serverSocket = new ServerSocket(5001, 3);
            InputStream in = null;
            while (true) {
                Socket socket = null;
                try {
    ...
    public interface EmergencyAlarmText {
        public void getListern(AlarmBen alarmBen);
    }
}
