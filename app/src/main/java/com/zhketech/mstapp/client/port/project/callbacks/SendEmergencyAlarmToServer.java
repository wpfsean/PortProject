...
public class SendEmergencyAlarmToServer extends Thread {

    VideoBen videoBen;
    Callback callback;
    String type;

    public SendEmergencyAlarmToServer(VideoBen videoBen, String type, Callback callback) {
    ...
    public void run() {
        ...
        try {
            alertS =type.getBytes(AppConfig.dataFormat);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        ...
        try {
            socket = new Socket(AppConfig.alarm_server_ip, 5001);
